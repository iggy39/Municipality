  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .mapActiveFilters {
      ...
      display: grid;
      gap: 6px;
      max-width: min(460px, calc(100% - 170px));
      padding: 8px 9px 9px;
    ...
    .mapLegendStack .mapActiveFilters {
      position: relative;
      inset: auto;
      z-index: auto;
      max-width: 100%;
    ...
    .mapActiveFilters[hidden] { display: none; }

    .mapActiveFiltersTitle {
      color: #475569;
      font-size: 10px;
      font-weight: 950;
    ...
    .mapActiveFilterChip.isArea {
      border-color: rgba(124, 77, 206, 0.24);
      background: rgba(245, 243, 255, 0.95);
      color: #5b21b6;
    }

    .mapActiveFilterChip.isMore {
    ...
    .mapFrame.hasRealGis .legendSourceTitle { display: none; }
    ...
    @media (max-width: 760px) {
      ...
      .mapLayerPopover { width: min(360px, calc(100% - 20px)); max-height: calc(100% - 20px); }
    }
  </style>
</head>
<body>
      ...
      <section class="mainCivicWorkspace" aria-label="מפה וציר זמן">
        <section class="mapPanel panelCard" aria-label="מפה סכמטית">
          ...
          <div class="mapFrame">
            ...
            <div class="mapLegendStack" aria-label="מידע על שכבות המפה">
              <div id="map-active-filters" class="mapActiveFilters" aria-live="polite" hidden>
                <span class="mapActiveFiltersTitle">מסננים פעילים במפה</span>
                <span class="mapActiveFilterChips" data-map-active-filter-chips></span>
              </div>
              ...
              <section class="legendCard" aria-label="מקרא מפה">
                ...
                <ul class="legendList" aria-label="סמלי מפה">
                  <li class="legendItem"><span class="legendSwatch area"></span><span>אזור נבחר</span></li>
                  <li class="legendItem"><span class="legendSwatch transit"></span><span>שכבת GovMap פעילה</span></li>
                  <li class="legendItem"><span class="legendSwatch interest"></span><span>סימון GovMap רשמי</span></li>
                </ul>
                <h2 class="legendTitle legendSourceTitle">מקורות</h2>
...
<!-- SOURCE_TYPE_OPTIONS -->
  ...
  <script>
    (() => {
      ...
      const legendSwatchClass = (row = {}) => {
        const id = String(row.id || row.key || "").toLowerCase();
        const kind = String(row.kind || "").toLowerCase();
        if (id.includes("selected") || id.includes("parcel") || kind === "polygon") return "area";
        if (id.includes("transport") || id.includes("bus")) return "transit";
        if (id.includes("school") || id.includes("public")) return "school";
        if (id.includes("neighborhood")) return "neighborhood";
        if (id.includes("park")) return "plan";
        return "interest";
      };

      const activeGovMapLayerCount = (payload = window.__municipalDashboardGisMap) => {
        const checked = Array.from(document.querySelectorAll('[data-resident-govmap-layer-toggle][data-resident-govmap-layer-kind="map_layer"]')).filter((input) => !input.disabled && input.checked).length;
        if (checked > 0) return checked;
        return Number(payload?.govmap?.default_visible_layers?.length || payload?.govmap?.question_visible_layers?.length || 0);
      };

      const replaceLegendRows = (rows) => {
        ...
        list.replaceChildren(...rows.map((row) => {
          ...
          swatch.className = `legendSwatch ${row.swatch || legendSwatchClass(row)}`;
          const label = document.createElement("span");
          label.textContent = row.label || "שכבת מפה";
      ...
      const applyRealGisLegendLabels = (payload = window.__municipalDashboardGisMap) => {
        if (payload?.provider === "govmap" || payload?.govmap?.enabled) {
          const activeCount = activeGovMapLayerCount(payload);
          const rows = [];
          if (selectedAreaFeature(payload) || payload?.govmap?.selected_area) {
            rows.push({ id: "selected_area", label: "אזור נבחר / רדיוס", swatch: "area" });
          }
          if (payload?.govmap?.address_marker) {
            rows.push({ id: "address_marker", label: "נקודת הכתובת", swatch: "interest" });
          }
          if (payload?.govmap?.neighborhood_boundary) {
            rows.push({ id: "neighborhood_boundary", label: "גבול שכונה אם זמין", swatch: "neighborhood" });
          }
          rows.push({ id: "active_govmap_layers", label: activeCount > 0 ? `שכבות GovMap פעילות (${activeCount})` : "שכבות GovMap פעילות", swatch: "transit" });
          rows.push({ id: "govmap_symbols", label: "סמלים ושמות מתוך GovMap", swatch: "interest" });
          return replaceLegendRows(rows.slice(0, 5));
        }
        const rows = Array.isArray(payload?.legend) ? payload.legend : [];
        if (rows.length > 0) {
          return replaceLegendRows(rows.slice(0, 5));
        }
        return false;
      ...
      const renderMapLayerCounts = (payload) => {
        ...
        if (payload.provider === "govmap" || payload.govmap?.enabled) {
          const activeLayerCount = activeGovMapLayerCount(payload);
          const rows = [
            ["מקור", { displayed_count: 1, count: 1, text: payload.basemap?.provider || "GovMap" }],
            ["שכבות", { displayed_count: activeLayerCount, count: activeLayerCount }],
          ];
          list.replaceChildren(...rows.map(([label, layer]) => {
            ...
            countNode.textContent = layer.text || String(layer.displayed_count || 0);
      ...
      window.__renderMapLayerCounts = renderMapLayerCounts;
      window.__applyRealGisLegendLabels = applyRealGisLegendLabels;
      ...
      const govMapCurrentAliases = (payload) => {
        ...
        const objectChecked = (key) => {
          const input = document.querySelector(`[data-map-object-toggle="${key}"]`);
          return !input || input.disabled || input.checked;
        };
      ...
      const renderResidentGovMapLayerGroups = (payload) => {
        ...
        for (const group of groups) {
          ...
          input.addEventListener("change", () => {
            ...
            window.__updateMapActiveFilterOverlay?.();
          });
      ...
      const applyQuestionGovMapLayerSelection = (payload = window.__municipalDashboardGisMap, explicitMapContext = null) => {
        ...
        for (const input of document.querySelectorAll("[data-resident-govmap-layer-group-toggle]")) {
          ...
          const expand = groupItem?.querySelector("[data-resident-govmap-expand]");
          input.checked = true;
          groupItem?.querySelectorAll("[data-resident-govmap-layer-toggle]").forEach((child) => {
            if (!child.disabled && aliases.size === 0) {
              child.checked = true;
            }
          });
      ...
      const syncGovMapNativeLayers = (payload = window.__municipalDashboardGisMap) => {
        ...
        if (!window.govmap?.setVisibleLayers || !payload?.govmap?.enabled || !nativeMap || nativeMap.hidden) {
          return false;
        }
      ...
      const syncGovMapSelectedArea = async (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        ...
        try {
          const rendered = await displayGovMapPolygonWkt({
            wkt: selectedWkt,
            name: geometryName,
            tooltip: "אזור נבחר",
            outlineColor: [124, 77, 206, 1],
          ...
          // GovMap can mount its iframe before displayGeometries is ready. Do not
          // permanently disable the selected-area filter on an early command timeout.
          setGovMapSelectedAreaNativeStatus(govMapNativeHasMountedContent(nativeMap) ? "unknown" : "failed");
          return false;
        }
      };

      const syncGovMapAddressMarker = async (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        ...
        try {
          return await displayGovMapHouseMarker({ marker, name: geometryName });
        } catch (error) {
          console.warn("govmap_address_marker_failed", error);
          return false;
        }
      };

      const syncGovMapNeighborhoodBoundary = async (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!nativeMap || nativeMap.hidden || !window.govmap) {
          return false;
        }
        ...
        try {
          return await displayGovMapPolygonWkt({
            wkt: boundaryWkt,
            name: geometryName,
          ...
          console.warn("govmap_neighborhood_boundary_failed", error);
          return false;
        }
      };

      const syncGovMapSelectedFocus = async (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        ...
        try {
          ...
          return requestSeq === govMapSelectedFocusRequestSeq;
        } catch (error) {
      ...
      const govMapNativeHasMountedContent = (nativeMap) => Boolean(nativeMap?.querySelector("iframe") || nativeMap?.childElementCount > 0);

      const syncGovMapNativeDecorations = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!payload?.govmap?.enabled || !nativeMap || nativeMap.hidden || !window.govmap) {
          return false;
        }
        syncGovMapSelectedArea(payload);
        syncGovMapAddressMarker(payload);
      ...
      const createNativeGovMap = async (payload) => {
        ...
        try {
          await waitForGovMapNativeCommand(govmap.createMap(GOVMAP_NATIVE_MAP_ID, {
          ...
          if (!govMapNativeHasMountedContent(nativeMap)) {
            throw error;
          }
          console.info("govmap_native_createMap_pending_after_mount", error);
        }
        syncGovMapNativeLayers(payload);
        window.setTimeout(() => {
          syncGovMapNativeDecorations(payload);
        }, 900);
        return true;
      };
      ...
      const renderGovMapEmbed = (payload) => {
        const mapNode = document.getElementById("real-gis-map");
        const mapFrame = document.querySelector(".mapFrame");
        const staticMap = document.getElementById("real-gis-static-map");
        ...
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const overlay = document.getElementById("govmap-dashboard-overlay");
        if (nativeMap) nativeMap.hidden = false;
        if (overlay) {
          overlay.hidden = true;
          overlay.replaceChildren();
        }
        ...
        let nativeDecorationRetriesScheduled = false;
        const scheduleGovMapNativeDecorationRetries = () => {
          if (nativeDecorationRetriesScheduled) return;
          nativeDecorationRetriesScheduled = true;
          for (const delayMs of [1800, 4000, 8000, 13000, 20000]) {
            window.setTimeout(() => {
              if (window.__municipalDashboardGisMap !== payload || !govMapNativeHasMountedContent(nativeMap)) return;
              syncGovMapNativeDecorations(payload);
            }, delayMs);
          }
        };
        const markGovMapNativeRendered = () => {
          if (!govMapNativeHasMountedContent(nativeMap)) return false;
          ...
          scheduleGovMapNativeDecorationRetries();
          return true;
        };
        const renderGovMapUnavailable = (error = null) => {
          ...
          setText("#map-renderer-status", "GovMap הרשמי לא נטען");
          if (dashboardRoot) dashboardRoot.dataset.gisMapStatus = "govmap-native-unavailable";
        };
        window.setTimeout(() => {
          if (window.__municipalDashboardGisMap === payload) {
            markGovMapNativeRendered();
          }
        }, 1800);
        window.setTimeout(() => {
          if (window.__municipalDashboardGisMap === payload) {
        ...
        renderMapLayerSummary(payload);
        applyRealGisLegendLabels();
        setText("#map-renderer-status", "טוען GovMap רשמי");
        ...
        setText(".mapProvenanceBadgeText", "המפה, הרקע והשכבות נטענים דרך GovMap הרשמי בלבד. אם GovMap לא נטען, לא מוצגת מפת גיבוי.");
      ...
      const loadDashboardGisMap = async (profile = "initial", options = {}) => {
        ...
        try {
          ...
          if (Number.isFinite(Number(options.center_x)) && Number.isFinite(Number(options.center_y)) && mapProvider === "govmap") {
      ...
      const questionGisMapRequest = (data) => {
        ...
        const municipality = String(data?.state?.municipality_id || selectedMunicipalityForQuery() || "").trim();
        const muniLabel = municipalitySearchLabel(municipality);
        const addressBase = String(focus.address_query || focus.place_query || "").trim();
        const address = [addressBase, muniLabel].filter(Boolean).join(" ").trim();
        ...
        if (!address && !govmapLayerAliases.length && !residentLayerKeys.length) return null;
        ...
        return { address, municipality, govmapLayerAliases, residentLayerKeys, focusLayer: geo.focus_layer, provider: "govmap", gisSignature };
      };
      ...
      const renderDashboardFromEndpoint = (data) => {
        ...
        setText(".legendTitle", document.querySelector(".mapFrame.hasRealGis") ? "מקרא" : mapCopy.legend_title);
  ...
  <script>
    (() => {
      ...
      const isGovMapActive = () => {
        const nativeMap = document.getElementById("govmap-native-map");
        const iframe = document.getElementById("govmap-iframe");
        return frame.classList.contains("govmapActive") && Boolean((nativeMap && !nativeMap.hidden) || (iframe && !iframe.hidden));
      };
      ...
      const mapActiveFiltersPanel = document.getElementById("map-active-filters");
      const mapActiveFilterChips = mapActiveFiltersPanel?.querySelector("[data-map-active-filter-chips]");
      const filterInputAvailable = (input) => Boolean(input && !input.disabled);
      const filterInputVisible = (input) => Boolean(input && !input.disabled && !input.closest("[hidden]"));
      const labelTextForFilterInput = (input) => {
        const label = input?.closest("label");
        if (!label) return "";
        const residentName = label.querySelector(".residentGovMapLayerName")?.textContent;
        if (residentName) return residentName.trim();
        const spans = Array.from(label.querySelectorAll("span")).map((node) => String(node.textContent || "").trim()).filter(Boolean);
        return spans.at(-1) || String(input.dataset.residentGovmapLayerToggle || input.dataset.mapLayerToggle || input.dataset.mapObjectToggle || "").trim();
      };
      const activeMapFilterRows = () => {
        const rows = [];
        const seen = new Set();
        const addRow = (label, kind = "layer") => {
          const text = String(label || "").replace(/\\s+/g, " ").trim();
          const key = `${kind}:${text}`;
          if (!text || seen.has(key)) return;
          seen.add(key);
          rows.push({ label: text, kind });
        };
        const parcelInput = document.querySelector('[data-map-layer-toggle="parcel"]');
        if (filterInputAvailable(parcelInput) && parcelInput.checked) {
          addRow(labelTextForFilterInput(parcelInput) || "אזור נבחר", "area");
        }
        for (const input of document.querySelectorAll('[data-resident-govmap-layer-toggle][data-resident-govmap-layer-kind="map_layer"]')) {
          if (filterInputAvailable(input) && input.checked) {
            addRow(labelTextForFilterInput(input), "layer");
          }
        }
        for (const input of document.querySelectorAll('[data-map-object-toggle]')) {
          if (filterInputVisible(input) && input.checked) {
            addRow(labelTextForFilterInput(input), "object");
          }
        }
        return rows;
      };
      const updateMapActiveFilterOverlay = () => {
        if (!mapActiveFiltersPanel || !mapActiveFilterChips) return false;
        const rows = activeMapFilterRows();
        const shouldShow = rows.length > 0 && Boolean(frame.classList.contains("hasRealGis") || isStaticMapActive() || isGovMapActive());
        mapActiveFiltersPanel.hidden = !shouldShow;
        mapActiveFilterChips.replaceChildren();
        if (!shouldShow) return false;
        const visibleRows = rows.slice(0, 5);
        for (const row of visibleRows) {
          const chip = document.createElement("span");
          chip.className = `mapActiveFilterChip${row.kind === "area" ? " isArea" : ""}`;
          chip.textContent = row.label;
          chip.title = row.label;
          mapActiveFilterChips.appendChild(chip);
        }
        if (rows.length > visibleRows.length) {
          const chip = document.createElement("span");
          chip.className = "mapActiveFilterChip isMore";
          chip.textContent = `+${rows.length - visibleRows.length}`;
          chip.title = rows.slice(visibleRows.length).map((row) => row.label).join(", ");
          mapActiveFilterChips.appendChild(chip);
        }
        if (window.__municipalDashboardGisMap) {
          window.__renderMapLayerCounts?.(window.__municipalDashboardGisMap);
          window.__applyRealGisLegendLabels?.(window.__municipalDashboardGisMap);
        }
        return true;
      };
      window.__updateMapActiveFilterOverlay = updateMapActiveFilterOverlay;
      ...
      controls.popover?.addEventListener("change", (event) => {
        if (event.__municipalDashboardFilterHandled) return;
        event.__municipalDashboardFilterHandled = true;
        const input = event.target.closest?.("[data-map-layer-toggle], [data-map-object-toggle], [data-govmap-layer-toggle], [data-resident-govmap-layer-toggle]");
