...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .headerButton.filterButton { min-width: 112px; }
    ...
    .dashboardGrid {
      direction: ltr;
      display: grid;
      grid-template-columns: minmax(360px, 0.28fr) minmax(0, 1fr) minmax(300px, 0.22fr);
      gap: 8px;
      width: 100%;
      margin: 0 auto;
    ...
    .appShell.isDetailCollapsed .dashboardGrid {
      grid-template-columns: 58px minmax(0, 1fr) minmax(300px, 0.22fr);
    }
    ...
    .appShell.isDetailCollapsed .endDetailDrawer {
      padding: 8px;
      overflow: hidden;
    }

    .appShell.isDetailCollapsed .endDetailDrawer > :not(.drawerTop) {
      display: none;
    }
    ...
    .mainCivicWorkspace {
      grid-column: 2;
      display: grid;
      grid-template-rows: minmax(0, 1fr) minmax(168px, 0.24fr);
      gap: 8px;
      min-width: 0;
      min-height: 0;
    }
    ...
    .appShell.isDetailCollapsed .drawerTop {
      justify-content: center;
      margin-bottom: 0;
    }
    ...
    .drawerCloseIconOpen { display: none; }

    .appShell.isDetailCollapsed .drawerCloseIconClose { display: none; }

    .appShell.isDetailCollapsed .drawerCloseIconOpen { display: block; }

    .appShell.isDetailCollapsed .detailTitle { display: none; }
    ...
    .residentContextStrip,
    .timelineProgressBanner {
      position: absolute;
      right: 12px;
      z-index: 2800;
      direction: rtl;
    ...
    .timelineProgressBanner[data-status="red"] .progressLamp { background: #dc2626; box-shadow: 0 0 0 4px rgba(220, 38, 38, 0.15); }
    .timelineProgressBanner[data-status="green"] .progressLamp { background: #16a34a; box-shadow: 0 0 0 4px rgba(22, 163, 74, 0.15); }
    .timelineProgressBanner[data-status="yellow"] .progressLamp { background: #f59e0b; box-shadow: 0 0 0 4px rgba(245, 158, 11, 0.16); }
    ...
    .mapIconSizeControl.isDisabled,
    .mapLayerLegendList label.isDisabled {
      opacity: 0.48;
      cursor: not-allowed;
    }

    .mapIconSizeControl.isDisabled input,
    .mapLayerLegendList label.isDisabled input {
      cursor: not-allowed;
    }
    ...
    .storyGovMapLayerItem label {
      min-height: 34px;
      padding: 6px 8px;
      align-items: center;
    }

    .storyGovMapLayerItem .residentGovMapLayerText {
      gap: 1px;
    }

    .storyGovMapLayerItem .storyActivatedBadge {
      display: none;
    }
    ...
    .govMapEmbedFrame {
      ...
      border: 0;
      background: #edf2ee;
      filter: saturate(0.82) contrast(0.93) brightness(1.04);
    }

    .govMapEmbedFrame[hidden] { display: none; }
    ...
    .govMapNativeMap[hidden] { display: none; }
    ...
    .govMapToneOverlay {
      ...
      pointer-events: none;
      background:
        linear-gradient(90deg, rgba(186, 230, 253, 0.26), transparent 26%),
        radial-gradient(circle at 72% 14%, rgba(255,255,255,0.44), transparent 20%);
    ...
    .govMapOverlay {
      ...
      pointer-events: none;
      direction: rtl;
      --govmap-icon-scale: 0.7;
      --govmap-label-scale: 0.7;
    }
    ...
    .govMapSelectedArea {
      fill: rgba(124, 77, 206, 0.24);
      stroke: rgba(124, 77, 206, 0.86);
      stroke-width: 2.4;
      filter: drop-shadow(0 8px 18px rgba(97, 61, 173, 0.18));
    }
    ...
    .govMapDistrictLabel {
      ...
      padding: 4px 10px;
      border-radius: 999px;
      background: rgba(124, 77, 206, 0.88);
      color: #fff;
      font-size: calc(13px * var(--govmap-label-scale, 0.7));
      font-weight: 900;
      box-shadow: 0 8px 20px rgba(81, 55, 146, 0.24);
      white-space: nowrap;
      pointer-events: none;
    }
    ...
    .govMapOverlayMarker {
      ...
      width: 26px;
      height: 26px;
      transform: translate(-50%, -50%) scale(var(--govmap-icon-scale, 0.7));
      display: inline-flex;
      align-items: center;
      justify-content: center;
      ...
      border-radius: 999px;
      color: #fff;
      box-shadow: 0 8px 18px rgba(22, 33, 54, 0.16), 0 0 0 4px rgba(255, 255, 255, 0.22);
      pointer-events: auto;
      cursor: pointer;
    }

    .govMapOverlayMarker svg {
      position: static;
      width: 15px;
      height: 15px;
      stroke-width: 2.25;
    ...
    .govMapOverlayMarker[data-kind="transport_stop"] { background: #0b68d1; }
    .govMapOverlayMarker[data-kind="school"] { background: #f97316; }
    .govMapOverlayMarker[data-kind="public_building"] { background: #f97316; }
    .govMapOverlayMarker[data-kind="environment"] { background: #18a865; }
    .govMapOverlayMarker[data-kind="emergency"] { background: #64748b; }

    .govMapOverlayMarker.isSelectedFocus {
      width: 34px;
      height: 34px;
      background: #7c3aed;
      border-color: #fff;
      box-shadow: 0 12px 30px rgba(76, 29, 149, 0.32), 0 0 0 7px rgba(124, 58, 237, 0.18);
      z-index: 5;
    }
    ...
    .gisStoryLane[data-lane="strong_story"] { border-block-start: 5px solid #16a34a; }
    .gisStoryLane[data-lane="needs_review"] { border-block-start: 5px solid #f59e0b; }
    ...
    .gisStoryCard[data-traffic="green"]::before { background: #16a34a; }
    .gisStoryCard[data-traffic="red"]::before { background: #dc2626; }
    ...
    .gisStoryCard[data-traffic="green"] .gisStoryTrafficDot { background: #16a34a; box-shadow: 0 0 0 3px rgba(22, 163, 74, 0.14); }
    .gisStoryCard[data-traffic="red"] .gisStoryTrafficDot { background: #dc2626; box-shadow: 0 0 0 3px rgba(220, 38, 38, 0.13); }
    ...
    .discoveryRow.isDisabled:hover,
    .treeRow.isDisabled:hover { background: #f8fafc; }
    ...
    @media (max-width: 1280px) {
      .dashboardGrid { grid-template-columns: 360px minmax(0, 1fr) 300px; }
      .municipalityBrand { min-width: 150px; }
      .brandWord { font-size: 30px; }
      .questionSearch { min-width: 360px; }
      .timelineCards { gap: 10px; }
    }
    ...
    @media (max-width: 1100px) {
      ...
      .appShell.isDetailCollapsed .dashboardGrid { grid-template-columns: 1fr; }
      .appShell.isDetailCollapsed .endDetailDrawer { min-height: 54px; }
    }
...
<body>
  <div class="appShell" id="rag-dashboard" data-dashboard-endpoint="/api/ui/rag-dashboard/mock" data-source-status="fallback" data-build-id="source-preview-text-only-v2">
    <header class="SearchHeader" aria-label="סרגל חיפוש ראשי">
      ...
      <form id="ask-playground-form" class="questionSearch" aria-label="שאלת חיפוש">
        <input id="ask-question" name="question" type="search" value="אילו החלטות עירייה התקבלו לאחרונה ליד דיזנגוף 99 תל אביב" aria-label="שאלה" autocomplete="off" />
    ...
    <main class="dashboardGrid" aria-label="מרחב עבודה עירוני">
      <aside class="endDetailDrawer panelCard" aria-label="תשובה">
        <div class="drawerTop">
          <button class="drawerClose" type="button" aria-label="סגירת תשובה" aria-expanded="true" aria-controls="detail-drawer-content">
            <svg class="icon drawerCloseIconClose" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
              <path d="M6 6l12 12M18 6 6 18" stroke-linecap="round" />
            </svg>
            <svg class="icon drawerCloseIconOpen" viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true">
        ...
        <div id="detail-drawer-content" class="detailDrawerContent">
        <p class="questionLine">שאלה: אילו החלטות עירייה התקבלו לאחרונה ליד דיזנגוף 99 תל אביב</p>
      ...
      <section class="mainCivicWorkspace" aria-label="מפה וציר זמן">
        <section class="mapPanel panelCard" aria-label="מפה סכמטית">
          ...
          <div id="resident-context-strip" class="residentContextStrip" aria-label="כתובת תושב">
            <span id="resident-context-label" class="residentContextLabel">מיקום</span>
            <strong id="resident-context-address">דיזנגוף 99 תל אביב</strong>
          </div>
          <div id="timeline-progress-banner" class="timelineProgressBanner" data-status="yellow" aria-live="polite">
            <span class="progressLamp" aria-hidden="true"></span>
            <span class="progressText">
              <span id="progress-meta" class="progressMeta">29.05.2024 · בבדיקה</span>
              <strong id="progress-title" class="progressTitle">סטטוס התקדמות</strong>
            </span>
          </div>
          <div class="mapFrame">
            ...
            <div id="real-gis-map" class="realGisMap" role="img" aria-label="מפת GovMap עם שכבות תושב" hidden>
              <div id="govmap-native-map" class="govMapNativeMap" aria-label="GovMap רשמי"></div>
              <iframe id="govmap-iframe" class="govMapEmbedFrame" title="GovMap - שכבות תושב" loading="lazy" referrerpolicy="origin" hidden></iframe>
              <div class="govMapToneOverlay" aria-hidden="true"></div>
              <div id="govmap-dashboard-overlay" class="govMapOverlay" aria-hidden="true" hidden></div>
            </div>
            ...
            <label class="mapExampleSelector" for="map-example-select">
              ...
              <select id="map-example-select" aria-label="בחירת דוגמת מפה">
                <option value="govmap_resident_point" selected>GovMap - נקודת תושב</option>
                <option value="tel_aviv_parcel">תל אביב - חלקת MAPI רשמית (גיבוי מקומי)</option>
              </select>
            </label>
  ...
  <section id="popular-popover" class="popularPopover" hidden aria-labelledby="popular-title">
    ...
    <button class="popularChoice" type="button">אילו החלטות עירייה התקבלו לאחרונה ליד דיזנגוף 99 תל אביב</button>
  </section>
  ...
  <script>
    (() => {
      ...
      const detailDrawerToggle = document.querySelector(".drawerClose");
      ...
      const APP_PATH_PREFIX = (() => {
        ...
        if (path.endsWith("/ui/ask")) {
          return path.slice(0, -"/ui/ask".length);
        }
        if (path.endsWith("/ask")) {
          return path.slice(0, -"/ask".length);
        }
        return "";
      })();
      const appUrl = (path) => `${APP_PATH_PREFIX}${path}` || path;
      const DASHBOARD_DATA_ENDPOINT = appUrl(dashboardRoot?.dataset.dashboardEndpoint || "/api/ui/rag-dashboard/mock");
      const DASHBOARD_QUERY_ENDPOINT = appUrl("/api/ui/rag-dashboard/query");
      const DASHBOARD_INTERACTION_ENDPOINT = appUrl("/api/ui/rag-dashboard/interaction");
      const DASHBOARD_EVIDENCE_ENDPOINT = appUrl("/api/ui/rag-dashboard/evidence");
      const DASHBOARD_GIS_MAP_ENDPOINT = appUrl("/api/ui/rag-dashboard/gis-map");
      const GOVMAP_IFRAME_CONFIG_ENDPOINT = appUrl("/api/govmap/iframe-config");
      const POINT_REPORT_ENDPOINT = appUrl("/v1/point-report");
      const COVERAGE_ENDPOINT = appUrl("/v1/coverage");
      const GOVMAP_API_SCRIPT_URL = "https://www.govmap.gov.il/govmap/api/govmap.api.js";
      const GOVMAP_NATIVE_MAP_ID = "govmap-native-map";
      const GOVMAP_SELECTED_NEIGHBORHOOD_LAYER = "neighborhoods_area";
      const GOVMAP_SELECTED_NEIGHBORHOOD_FIELD = "objectid";
      const GOVMAP_SELECTED_NEIGHBORHOOD_RADIUS_M = 12;
      ...
      let govMapSelectedAreaRequestSeq = 0;
      let govMapSelectedFocusRequestSeq = 0;
      ...
      let currentMapExample = mapExampleSelect?.value || "govmap_resident_point";
      let currentMapIconScale = 0.12;
      let currentMapLabelScale = 0.18;
      let gisMapProfileRequest = null;
      let lastQuestionGisMapSignature = "";
      let govMapSelectedAreaNativeStatus = "unknown";
      let govMapNativeReady = false;
      let govMapNativeCommandReady = false;
      let govMapNativeRenderSeq = 0;
      ...
      window.__govMapApiReady = window.__govMapApiReady || null;
      window.__municipalDashboardGovMapNativeReady = false;
      ...
      const selectedGisStoryIdFromData = (data = currentDashboardData) => String(data?.state?.selected_gis_story_id || dashboardState?.selected_gis_story_id || "").trim();
      ...
      const syncDetailDrawerToggle = (closeLabel = "סגירת תשובה") => {
        if (!detailDrawerToggle) return;
        detailDrawerToggle.setAttribute("aria-expanded", String(!detailDrawerCollapsed));
        detailDrawerToggle.setAttribute("aria-label", detailDrawerCollapsed ? "פתיחת תשובה" : closeLabel);
        detailDrawerToggle.title = detailDrawerCollapsed ? "פתיחת תשובה" : closeLabel;
      };

      const setDetailDrawerCollapsed = (collapsed) => {
        detailDrawerCollapsed = Boolean(collapsed);
        dashboardRoot?.classList.toggle("isDetailCollapsed", detailDrawerCollapsed);
        dashboardRoot?.setAttribute("data-detail-drawer-collapsed", String(detailDrawerCollapsed));
        syncDetailDrawerToggle();
      };
      ...
      const zoomDashboardMap = (delta) => {
        ...
        if (window.__zoomGovMapEmbed?.(direction)) {
          return false;
        }
      ...
      const recenterDashboardMap = () => {
        ...
        if (window.__recenterGovMapEmbed?.()) {
          return false;
        }
      ...
      const updateDecisionRows = (decisions, sourceLabel) => {
        ...
        for (let idx = 0; idx < rows.length; idx += 1) {
          ...
          if (link) {
            ...
            link.href = appUrl(`/api/ui/rag-dashboard/evidence/${encodeURIComponent(evidenceRef)}`);
      ...
      const ensureRowButtons = (selector, rows) => {
        const root = document.querySelector(selector);
        if (!root) {
          return [];
        }
        const list = root.classList.contains("treeChildren") ? root : (root.querySelector(".rowList") || root);
        const isTree = root.classList.contains("treeChildren");
        const isTopic = selector.includes("nth-of-type(2)") || isTree;
        let buttons = Array.from(root.querySelectorAll(".discoveryRow, .treeRow"));
        while (buttons.length < (Array.isArray(rows) ? rows.length : 0)) {
          const button = document.createElement("button");
          button.type = "button";
          button.className = isTree ? "treeRow" : `discoveryRow${isTopic ? " topicRow" : ""}`;
          if (!isTopic) {
      ...
      const updateRowList = (selector, rows) => {
        const buttons = ensureRowButtons(selector, rows);
        for (let idx = 0; idx < buttons.length; idx += 1) {
          ...
          buttons[idx].classList.toggle("isDisabled", disabled);
          buttons[idx].setAttribute("aria-disabled", disabled ? "true" : "false");
          buttons[idx].title = disabled ? (row.disabled_reason || "לא רלוונטי לשאלה הנוכחית") : "";
      ...
      const storyTrafficLabel = (value) => ({ green: "ירוק", yellow: "צהוב", red: "אדום" }[value] || "צהוב");
      ...
      const createGisStoryCard = (story) => {
        ...
        traffic.className = "gisStoryTraffic";
        const dot = document.createElement("span");
        dot.className = "gisStoryTrafficDot";
        dot.setAttribute("aria-hidden", "true");
        const trafficText = document.createElement("span");
        trafficText.textContent = storyTrafficLabel(story.traffic_light);
        traffic.append(dot, trafficText);
        const meta = document.createElement("span");
        meta.className = "gisStoryMeta";
        meta.textContent = `${Number(story.event_count || 0)} events · ${Number(story.source_protocol_count || 0)} protocols`;
        top.append(traffic, meta);

        const title = document.createElement("h4");
        title.className = "gisStoryCardTitle";
        title.textContent = story.title_he || "סיפור ללא כותרת";

        const tags = document.createElement("div");
        tags.className = "gisStoryTags";
        for (const value of (Array.isArray(story.subject_terms) ? story.subject_terms : []).slice(0, 4)) {
          const tag = document.createElement("span");
          tag.className = "gisStoryTag";
          tag.textContent = value;
          tags.append(tag);
        }

        const foot = document.createElement("div");
        foot.className = "gisStoryFoot";
        const dateRange = document.createElement("span");
        dateRange.innerHTML = `<strong>${escapeHtml(story.first_event_label || "?")} → ${escapeHtml(story.latest_event_label || "?")}</strong>ציר זמן`;
        const progression = document.createElement("span");
        progression.innerHTML = `<strong>${escapeHtml(story.progression || "unknown")}</strong>התקדמות`;
        const confidence = document.createElement("span");
        confidence.innerHTML = `<strong>${escapeHtml(story.judge_confidence || "")}</strong>שיפוט`;
        foot.append(dateRange, progression, confidence);

        item.append(top, title, tags, foot);
        if (story.mock_warning) {
          const warning = document.createElement("span");
          warning.className = "gisStoryWarning";
          warning.textContent = "כולל תאריכים/שדות מוקאפ";
          item.append(warning);
        }
        return item;
      };

      const renderGisStoryReview = (storyReview) => {
        const panel = document.getElementById("gis-story-review-panel");
        if (!panel) return;
        const lanes = Array.isArray(storyReview?.lanes) ? storyReview.lanes : [];
        panel.hidden = lanes.length === 0;
        if (!lanes.length) return;
        const summary = storyReview.summary || {};
        setText("#gis-story-review-title", storyReview.title_he || "סיפורי GIS לפרוטוקולים");
        setText("#gis-story-review-subtitle", storyReview.subtitle_he || "");
        setText("#gis-story-total", summary.story_count ?? 0);
        setText("#gis-story-strong-count", summary.strong_story_count ?? 0);
        setText("#gis-story-review-count", summary.needs_review_count ?? 0);

        const strongLane = lanes.find((lane) => lane.id === "strong_story") || {};
        const reviewLane = lanes.find((lane) => lane.id === "needs_review") || {};
        setText("#gis-story-strong-title", strongLane.title_he || "מוכן לדמו");
        setText("#gis-story-needs-title", reviewLane.title_he || "דורש בדיקה");
        const strongCount = document.getElementById("gis-story-strong-lane-count");
        const reviewCount = document.getElementById("gis-story-needs-lane-count");
        if (strongCount) strongCount.textContent = String(strongLane.count ?? (strongLane.stories || []).length ?? 0);
        if (reviewCount) reviewCount.textContent = String(reviewLane.count ?? (reviewLane.stories || []).length ?? 0);
        setText("#gis-story-strong-desc", strongLane.description_he || "");
        setText("#gis-story-needs-desc", reviewLane.description_he || "");

        const strongList = document.getElementById("gis-story-strong-list");
        const reviewList = document.getElementById("gis-story-needs-list");
        if (strongList) {
          strongList.replaceChildren(...(strongLane.stories || []).slice(0, 9).map(createGisStoryCard));
        }
        if (reviewList) {
          reviewList.replaceChildren(...(reviewLane.stories || []).slice(0, 9).map(createGisStoryCard));
        }
        if (dashboardRoot) {
          dashboardRoot.dataset.gisStoryReviewStatus = storyReview.status || "";
          dashboardRoot.dataset.gisStoryStrongCount = String((strongLane.stories || []).length || 0);
          dashboardRoot.dataset.gisStoryNeedsReviewCount = String((reviewLane.stories || []).length || 0);
        }
      };
      ...
      const mapObjectType = (category) => {
        ...
        if (category === "environment") return "municipal_parks";
        if (category === "emergency") return "municipal_pois";
        if (category === "public_building") return "municipal_pois";
      ...
      const mapExampleCopy = (example) => ({
        govmap_resident_point: "GovMap משמש כמקור GIS ראשי: שכבות רשמיות, חיפוש כתובות ורחובות, ותצוגת שכבות סביב נקודת התושב.",
        tel_aviv_parcel: "גיבוי מקומי בלבד: חלקת MAPI רשמית, שכבות PostGIS מקומיות ו-OSM כהקשר."
      }[example] || "GovMap משמש כמקור GIS ראשי לשאלות תושב.");
      ...
      const selectedMunicipalityForQuery = () => {
        ...
        if (code && municipalityLabelBySlug[code]) {
          return code;
        }
        return String(dashboardState?.municipality_id || currentDashboardData?.state?.municipality_id || "").trim() || undefined;
      };

      const municipalitySearchLabel = (slug) => municipalityLabelBySlug[String(slug || "").trim()] || String(slug || "").replaceAll("_", " ").trim();
      ...
      const mapExampleLayers = (example) => {
        ...
        const configs = {
          govmap_resident_point: { nearbyParcels: true, pois: true, plans: true, boundaries: true, neighborhoods: true, addressPoints: true, municipalPois: true, contextPois: true, buildings: false, basemap: true },
        ...
        return { ...base, ...(configs[example] || configs.govmap_resident_point) };
      };
      ...
      const mapContextLayerLabel = (layerKey) => ({
        ...
        buildings: "מבנים OSM",
        resident_location: "כתובת תושב",
        neighborhoods_and_statistics: "שכונות וסטטיסטיקה",
        parcels_cadaster: "גושים וחלקות",
        planning_land_use: "תכנון וייעודי קרקע",
        public_buildings_assets: "מבני ציבור ונכסים",
        education_facilities: "בתי ספר וגנים",
        playgrounds_youth_space: "פארקים ונוער",
        welfare_health_services: "בריאות ורווחה",
        emergency_security_services: "חירום וביטחון",
        public_transport_access: "תחבורה ציבורית",
        roads_parking_public_works: "דרכים וחניה",
        drainage_water_sewer: "ניקוז ומים",
        environment_sanitation: "סביבה וניקיון",
        cellular_and_radiation_context: "אנטנות וסלולר",
        culture_sport_leisure: "תרבות וספורט",
        religious_services: "שירותי דת",
        commerce_employment: "מסחר ותעסוקה",
      ...
      const renderDashboardMapContext = (mapContext) => {
        ...
        const residentContext = mapContext.resident_context || {};
        setText("#resident-context-label", residentContext.label_he || "מיקום");
        setText("#resident-context-address", residentContext.address || "");
        const progress = mapContext.progress || {};
        const progressBanner = document.getElementById("timeline-progress-banner");
        if (progressBanner) {
          progressBanner.dataset.status = progress.status || "yellow";
        }
        ...
        if (dashboardRoot) {
          ...
          dashboardRoot.dataset.timelineProgressStatus = progress.status || "";
          dashboardRoot.dataset.residentAddress = residentContext.address || "";
        }
      };
      ...
      const renderStaticGisMap = (payload, options = {}) => {
        ...
        if (shouldRenderMapItem("parcel", "parcels")) {
          ...
          if (selectedArea?.geometry) {
            appendStaticPolygonFeature(svg, selectedArea, project, { "data-map-layer": "parcel", "data-map-object": "selected_area", fill: "rgba(124, 77, 206, 0.20)", stroke: "rgba(255,255,255,0.96)", "stroke-width": "7", "stroke-linejoin": "round" }, "אזור נבחר · מעגל סביב הכתובת");
            appendStaticPolygonFeature(svg, selectedArea, project, { "data-map-layer": "parcel", "data-map-object": "selected_area", fill: "rgba(124, 77, 206, 0.22)", stroke: "#7c4dce", "stroke-width": "2.6", "stroke-linejoin": "round" }, "אזור נבחר · מעגל סביב הכתובת");
          }
      ...
      const markerSvg = (kind) => {
        ...
        if (kind === "public_building") {
          return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path d="M4 20h16M6 20V9l6-4 6 4v11M9 20v-6h6v6M8 11h.01M12 11h.01M16 11h.01" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        }
        if (kind === "emergency") {
          return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" aria-hidden="true"><path d="M12 4v16M4 12h16" stroke-linecap="round"/><circle cx="12" cy="12" r="8"/></svg>';
        }
      ...
      const ensureGovMapApi = () => {
        if (window.govmap?.createMap) {
          return Promise.resolve(window.govmap);
        }
        if (window.__govMapApiReady) {
          return window.__govMapApiReady;
        }
        window.__govMapApiReady = new Promise((resolve, reject) => {
          const existing = document.querySelector(`script[src="${GOVMAP_API_SCRIPT_URL}"]`);
          const script = existing || document.createElement("script");
          script.src = GOVMAP_API_SCRIPT_URL;
          script.async = true;
          script.onload = () => window.govmap?.createMap ? resolve(window.govmap) : reject(new Error("govmap_api_missing_createMap"));
          script.onerror = () => reject(new Error("govmap_api_load_failed"));
          if (!existing) {
      ...
      const loadGovMapIframeConfig = async () => {
        try {
          const response = await fetch(GOVMAP_IFRAME_CONFIG_ENDPOINT, { cache: "no-store", headers: { Accept: "application/json" } });
          const payload = await response.json().catch(() => ({}));
          if (!response.ok) {
            console.warn("govmap_iframe_config_unavailable", payload);
            return null;
          }
          return payload;
        } catch (error) {
          console.warn("govmap_iframe_config_failed", error);
          return null;
        }
      };

      const govMapCurrentAliases = (payload) => {
        const storyFiltersEnabled = payload?.govmap?.story_filters_enabled === true && Boolean(String(payload?.govmap?.selected_gis_story_id || selectedGisStoryIdFromData()).trim());
        const aliases = new Set();
        if (!storyFiltersEnabled) return [];
        const residentLayerInputs = Array.from(document.querySelectorAll("[data-resident-govmap-layer-toggle]"));
        if (!residentLayerInputs.length) {
          for (const alias of (payload?.govmap?.question_visible_layers || [])) {
        ...
        for (const input of residentLayerInputs) {
      ...
      const govMapAllAliases = (payload = window.__municipalDashboardGisMap) => Array.from(new Set(payload?.govmap?.visible_layers || []));

      const storyCompatibleGovMapAliases = (payload = window.__municipalDashboardGisMap, explicitMapContext = null) => {
        if (!String(payload?.govmap?.selected_gis_story_id || selectedGisStoryIdFromData()).trim()) return new Set();
        ...
        return new Set([
          ...(payload?.govmap?.question_visible_layers || []),
          ...storyLayers.flatMap((layer) => Array.isArray(layer?.govmap_aliases) ? layer.govmap_aliases : []),
        ].map((alias) => String(alias || "").trim()).filter(Boolean));
      };

      const setControlDisabled = (input, disabled, reason = "") => {
        if (!input) return;
        input.disabled = disabled;
        if (reason) input.title = disabled ? reason : "";
        const label = input.closest("label");
        label?.classList.toggle("isDisabled", disabled);
        if (reason && label) label.title = disabled ? reason : "";
      };

      const updateGovMapFilterAvailability = (active) => {
        ...
        for (const input of document.querySelectorAll("[data-map-layer-toggle]")) {
          ...
          setControlDisabled(input, active && (input.dataset.mapLayerToggle !== "parcel" || selectedAreaUnavailable), selectedAreaUnavailable ? "האזור הנבחר יוצג רק כאשר GovMap מאשר פקודות API לדומיין הנוכחי." : disabledLayerReason);
        }
        const nonOperativeObjects = new Set(["buildings", "osm_roads", "osm_railways", "addresses", "municipal_parks", "osm_parks", "municipal_beaches", "osm_water", "municipal_bike_paths", "osm_interest"]);
        const objectReason = "מסנן זה אינו מחובר לשכבת GovMap רשמית במפה הנוכחית.";
        for (const input of document.querySelectorAll("[data-map-object-toggle]")) {
          setControlDisabled(input, active && nonOperativeObjects.has(input.dataset.mapObjectToggle), objectReason);
        }
      };
      ...
      const renderResidentGovMapLayerGroups = (payload) => {
        ...
        const compatibleAliases = storyCompatibleGovMapAliases(payload);
        const compatibleStoryLayers = [];
        const compatibleLayerAliases = new Set();
        for (const group of groups) {
          for (const layer of (Array.isArray(group?.layers) ? group.layers : [])) {
            const alias = String(layer?.alias || "").trim();
            if (alias && layer?.selectable !== false && layer?.kind === "map_layer" && compatibleAliases.has(alias)) {
              compatibleLayerAliases.add(alias);
              compatibleStoryLayers.push({ ...layer, alias, group_name_he: group.display_name_he || group.layer_key || "GovMap" });
            }
          }
        }
        const nativeReady = payload?.govmap?.native_ready === true && govMapNativeReady === true;
        const selectedStoryReady = payload?.govmap?.story_filters_enabled === true && Boolean(String(payload?.govmap?.selected_gis_story_id || selectedGisStoryIdFromData()).trim()) && compatibleLayerAliases.size > 0;
        const storyFiltersEnabled = selectedStoryReady;
        if (help && groups.length) {
          help.textContent = storyFiltersEnabled
            ? "השכבות הפעילות הוגבלו להתאמות GovMap אמיתיות של הסיפור הנבחר."
            : selectedStoryReady
              ? "מסנני השכבות יופעלו כאשר מפת GovMap הרשמית פעילה."
              : "בחר סיפור GIS עם התאמות GovMap כדי להפעיל מסנני שכבות.";
        }
        ...
        const updateSearchVisibility = () => {
          ...
          if (resultCount) {
            resultCount.textContent = selectedStoryReady && !query ? `${compatibleLayerAliases.size} GovMap layers active for this story` : query ? `${visibleCount} קבוצות נמצאו` : `${visibleCount} קבוצות זמינות`;
          }
        ...
        if (selectedStoryReady) {
          for (const layer of compatibleStoryLayers) {
            const alias = String(layer.alias || "").trim();
            if (!alias) continue;
            const item = document.createElement("li");
            item.className = "residentGovMapGroupItem storyGovMapLayerItem";
            item.dataset.groupName = layer.group_name_he || "GovMap";
            item.dataset.searchText = [layer.group_name_he, alias, layer.label_he, layer.kind].filter(Boolean).join(" ");
            const label = document.createElement("label");
            label.className = "isStoryActivated";
            if (layer.evidence_he || layer.caveat_he) {
              label.title = [layer.evidence_he, layer.caveat_he].filter(Boolean).join(" ");
            }
            const input = document.createElement("input");
            input.type = "checkbox";
            input.checked = false;
            input.disabled = false;
            input.dataset.residentGovmapLayerToggle = alias;
            input.dataset.residentGovmapLayerKind = layer.kind || "map_layer";
            ...
            name.textContent = String(layer.label_he || "").trim() || alias;
            const meta = document.createElement("span");
            meta.className = "residentGovMapLayerMeta";
            meta.textContent = `${alias} · ${layer.group_name_he || "GovMap"}`;
            ...
            activated.className = "storyActivatedBadge";
            activated.textContent = "Activated by selected story";
            textWrap.appendChild(activated);
        ...
        const groupCompatibleLayerCount = (group) => (Array.isArray(group?.layers) ? group.layers : []).filter((layer) => compatibleLayerAliases.has(String(layer?.alias || "").trim())).length;
        const orderedGroups = selectedStoryReady ? [...groups].sort((left, right) => groupCompatibleLayerCount(right) - groupCompatibleLayerCount(left)) : groups;
        for (const group of orderedGroups) {
          const key = String(group.layer_key || "").trim();
          const layers = (Array.isArray(group.layers) ? group.layers : []).filter((layer) => !selectedStoryReady || compatibleLayerAliases.has(String(layer?.alias || "").trim()));
          ...
          for (const layer of layers) {
            ...
            const selectable = layer.selectable !== false && layer.kind === "map_layer" && storyFiltersEnabled && compatibleAliases.has(alias);
            ...
            childName.textContent = rawLabel || alias;
            ...
            if (!selectable) {
              ...
              childLabel.title = !nativeReady && selectedStoryReady ? "GovMap הרשמי עדיין לא פעיל. אין תצוגת גיבוי." : storyFiltersEnabled ? (layer.reason_he || "שכבה זו אינה תואמת לסיפור הנבחר.") : "בחר סיפור GIS תואם כדי להפעיל שכבות GovMap.";
      ...
      const applyQuestionGovMapLayerSelection = (payload = window.__municipalDashboardGisMap, explicitMapContext = null) => {
        ...
        for (const input of document.querySelectorAll("[data-resident-govmap-layer-group-toggle]")) {
          const key = String(input.dataset.residentGovmapLayerGroupToggle || "").trim();
          if (!layerKeys.has(key)) continue;
          const groupItem = input.closest(".residentGovMapGroupItem");
          const expand = groupItem?.querySelector("[data-resident-govmap-expand]");
          const hasActivatedChild = Array.from(groupItem?.querySelectorAll("[data-resident-govmap-layer-toggle]") || []).some((child) => aliases.has(String(child.dataset.residentGovmapLayerToggle || "").trim()));
          if (hasActivatedChild && expand && expand.getAttribute("aria-expanded") !== "true") {
            expand.click();
          }
        }
        window.__syncResidentGovMapLayerGroups?.();
        window.setTimeout(() => syncGovMapNativeLayers(payload), 0);
        window.setTimeout(() => syncGovMapNativeLayers(payload), 700);
        return true;
      };

      const syncGovMapNativeLayers = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!payload?.govmap?.enabled || !nativeMap || nativeMap.hidden) {
          return false;
        }
        const on = govMapCurrentAliases(payload);
        const onSet = new Set(on);
        const off = govMapAllAliases(payload).filter((alias) => !onSet.has(alias));
        const iframeUpdated = updateMountedGovMapIframeUrl(payload);
        if (!govMapNativeCommandReady || !window.govmap?.setVisibleLayers) {
          return iframeUpdated;
        }
        try {
          window.govmap.setVisibleLayers(on, off, GOVMAP_NATIVE_MAP_ID);
          syncGovMapSelectedArea(payload);
          syncGovMapAddressMarker(payload);
          syncGovMapNeighborhoodBoundary(payload);
          syncGovMapSelectedFocus(payload);
          return true;
        } catch (error) {
          console.warn("govmap_layer_visibility_sync_failed", error);
          return iframeUpdated;
        }
      };

      const selectedAreaChecked = () => document.querySelector('[data-map-layer-toggle="parcel"]')?.checked !== false;
      const govMapLayerToggleChecked = (alias) => {
        const text = String(alias || "").trim();
        if (!text) return true;
        const input = document.querySelector(`[data-resident-govmap-layer-toggle="${text}"]`);
        return !input || input.checked;
      };

      const setGovMapSelectedAreaNativeStatus = (status, reason = "") => {
        govMapSelectedAreaNativeStatus = status;
        const input = document.querySelector('[data-map-layer-toggle="parcel"]');
        if (!input) return;
        const unavailable = status === "failed" || status === "unavailable";
        setControlDisabled(input, unavailable, unavailable ? (reason || "האזור הנבחר יוצג רק כאשר GovMap מאשר פקודות API לדומיין הנוכחי.") : "");
      };

      const waitForGovMapNativeCommand = (command, timeoutMs = 4500) => new Promise((resolve, reject) => {
        let settled = false;
        const timer = window.setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error("govmap_native_command_timeout"));
        }, timeoutMs);
        Promise.resolve(command).then((value) => {
          if (settled) return;
          settled = true;
      ...
      const govMapGeometryNamePart = (value) => String(value || "selected").replace(/[^A-Za-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 90) || "selected";

      const displayGovMapPolygonWkt = async ({ wkt, name, tooltip, outlineColor, outlineWidth, fillColor }) => {
        if (!wkt || !name || !window.govmap?.displayGeometries) {
          return false;
        }
        window.govmap.clearGeometriesByName?.([name], GOVMAP_NATIVE_MAP_ID);
        await waitForGovMapNativeCommand(window.govmap.displayGeometries({
          wkts: [wkt],
          names: [name],
          geometryType: window.govmap.geometryType?.POLYGON ?? 2,
          clearExisting: false,
          showBubble: false,
          defaultSymbol: {
            outlineColor,
          ...
          data: { tooltips: [tooltip || "אזור נבחר"] },
        }, GOVMAP_NATIVE_MAP_ID));
        return true;
      };

      const displayGovMapPointWkt = async ({ wkt, name, tooltip }) => {
        if (!wkt || !name || !window.govmap?.displayGeometries) {
          return false;
        }
        window.govmap.clearGeometriesByName?.([name], GOVMAP_NATIVE_MAP_ID);
        await waitForGovMapNativeCommand(window.govmap.displayGeometries({
          wkts: [wkt],
          names: [name],
          geometryType: window.govmap.geometryType?.POINT ?? 0,
          clearExisting: false,
          showBubble: false,
          defaultSymbol: {
          ...
          data: { tooltips: [tooltip || "כתובת"] },
        }, GOVMAP_NATIVE_MAP_ID));
        return true;
      };

      const displayGovMapHouseMarker = async ({ marker, name }) => {
        if (!marker || !name) {
          return false;
        }
      ...
      const govMapWhereValue = (value) => {
        const text = String(value ?? "").trim();
        if (/^\\d+$/.test(text)) return text;
        return `'${text.replaceAll("'", "''")}'`;
      };

      const collectGovMapObjectIds = (value, output = [], depth = 0) => {
        if (!value || output.length >= 3 || depth > 6) return output;
        if (Array.isArray(value)) {
          for (const item of value) collectGovMapObjectIds(item, output, depth + 1);
          return output;
        }
        if (typeof value !== "object") return output;
        const readId = (source) => {
          if (!source || typeof source !== "object") return;
          const raw = source[GOVMAP_SELECTED_NEIGHBORHOOD_FIELD] ?? source.OBJECTID ?? source.ObjectId ?? source.objectId;
          const text = String(raw ?? "").trim();
          if (text && !output.includes(text)) output.push(text);
        };
        readId(value.attributes || value.Attributes || value.properties || value.Properties || value.fields || value.Fields);
        readId(value);
        if (String(value.idFieldName || value.id_field_name || "").toLowerCase() === GOVMAP_SELECTED_NEIGHBORHOOD_FIELD) {
          const text = String(value.id ?? "").trim();
          if (text && !output.includes(text)) output.push(text);
        }
        for (const child of Object.values(value)) collectGovMapObjectIds(child, output, depth + 1);
        return output;
      };

      const resolveGovMapSelectedNeighborhoodId = async (payload) => {
        const existing = payload?.govmap?.selected_area?.objectid || payload?.govmap?.selected_neighborhood_object_id;
        if (existing) return String(existing);
        if (payload?.govmap?.selected_neighborhood_promise) return payload.govmap.selected_neighborhood_promise;
        const center = payload?.govmap?.center;
        if (!center || !window.govmap?.selectFeaturesOnMap) return null;
        const pointWkt = `POINT(${Number(center.x)} ${Number(center.y)})`;
        payload.govmap.selected_neighborhood_promise = Promise.resolve(window.govmap.selectFeaturesOnMap({
          continous: false,
          wkt: pointWkt,
          radius: GOVMAP_SELECTED_NEIGHBORHOOD_RADIUS_M,
          getWkt: true,
          filterLayer: false,
          layers: [GOVMAP_SELECTED_NEIGHBORHOOD_LAYER],
          returnFields: { [GOVMAP_SELECTED_NEIGHBORHOOD_LAYER]: [GOVMAP_SELECTED_NEIGHBORHOOD_FIELD] },
          selectOnMap: false,
          isZoomToExtent: false,
        }, GOVMAP_NATIVE_MAP_ID)).then((result) => {
          const ids = collectGovMapObjectIds(result);
          const objectId = ids.length === 1 ? ids[0] : null;
          if (objectId) payload.govmap.selected_neighborhood_object_id = objectId;
          if (!objectId) throw new Error(`govmap_selected_neighborhood_expected_one_found_${ids.length}`);
          return objectId;
        }).catch((error) => {
          delete payload.govmap.selected_neighborhood_promise;
          throw error;
        });
        return payload.govmap.selected_neighborhood_promise;
      };

      const highlightGovMapSelectedNeighborhood = async (objectId) => {
        const center = window.__municipalDashboardGisMap?.govmap?.center;
        if (!objectId || !center || !window.govmap) return false;
        window.govmap.clearSelection?.(GOVMAP_NATIVE_MAP_ID);
        window.govmap.clearGeometriesByName?.([`selected-neighborhood-${objectId}`], GOVMAP_NATIVE_MAP_ID);
        const idText = String(objectId);
        const selectedArea = window.__municipalDashboardGisMap?.govmap?.selected_area || {};
        const selectedWkt = selectedArea.display_wkt || selectedArea.wkt;
        if (selectedWkt && window.govmap.displayGeometries) {
          await waitForGovMapNativeCommand(window.govmap.displayGeometries({
            wkts: [selectedWkt],
            names: [`selected-neighborhood-${idText}`],
            geometryType: window.govmap.geometryType?.POLYGON ?? 2,
            clearExisting: false,
            showBubble: false,
            defaultSymbol: {
              outlineColor: [124, 77, 206, 1],
              outlineWidth: 3,
              fillColor: [124, 77, 206, 0.28],
            },
            data: { tooltips: ["אזור נבחר"] },
          }, GOVMAP_NATIVE_MAP_ID));
          return true;
        }
        if (!window.govmap.selectFeaturesOnMap) return false;
        const pointWkt = `POINT(${Number(center.x)} ${Number(center.y)})`;
        await waitForGovMapNativeCommand(window.govmap.selectFeaturesOnMap({
          continous: false,
          wkt: pointWkt,
          radius: GOVMAP_SELECTED_NEIGHBORHOOD_RADIUS_M,
          getWkt: true,
          ...
          returnFields: { [GOVMAP_SELECTED_NEIGHBORHOOD_LAYER]: [GOVMAP_SELECTED_NEIGHBORHOOD_FIELD] },
          selectOnMap: true,
          whereClause: { [GOVMAP_SELECTED_NEIGHBORHOOD_LAYER]: `(${GOVMAP_SELECTED_NEIGHBORHOOD_FIELD} = ${govMapWhereValue(idText)})` },
          isZoomToExtent: false,
        }, GOVMAP_NATIVE_MAP_ID));
        return true;
      };

      const syncGovMapSelectedArea = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!nativeMap || nativeMap.hidden || !window.govmap) {
          return false;
        }
        const selectedArea = payload?.govmap?.selected_area || payload?.parcel;
        const geometryName = `selected-area-${govMapGeometryNamePart(selectedArea?.id || selectedArea?.provenance_id || "address-circle")}`;
        if (!selectedAreaChecked()) {
          govMapSelectedAreaRequestSeq += 1;
          window.govmap.clearGeometriesByName?.([geometryName], GOVMAP_NATIVE_MAP_ID);
          return true;
        }
        const selectedWkt = selectedArea?.display_wkt || selectedArea?.wkt;
        if (!payload?.govmap || !selectedWkt) {
          return false;
        }
        const requestSeq = ++govMapSelectedAreaRequestSeq;
        displayGovMapPolygonWkt({
          wkt: selectedWkt,
          name: geometryName,
          tooltip: "אזור נבחר",
          outlineColor: [124, 77, 206, 1],
          outlineWidth: 3,
          fillColor: [124, 77, 206, 0.22],
        })
          .then((rendered) => {
            if (requestSeq !== govMapSelectedAreaRequestSeq || !selectedAreaChecked()) return false;
            setGovMapSelectedAreaNativeStatus(rendered ? "ready" : "unavailable");
          })
          .catch((error) => {
            if (requestSeq !== govMapSelectedAreaRequestSeq || !selectedAreaChecked()) return;
            console.warn("govmap_selected_area_circle_failed", error);
            setGovMapSelectedAreaNativeStatus("failed");
          });
        return true;
      };

      const syncGovMapAddressMarker = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const marker = payload?.govmap?.address_marker;
        if (!nativeMap || nativeMap.hidden || !window.govmap || !marker?.display_wkt) {
          return false;
        }
        const geometryName = `address-marker-${govMapGeometryNamePart(marker.id || marker.provenance_id || "address")}`;
        displayGovMapHouseMarker({ marker, name: geometryName }).catch((error) => console.warn("govmap_address_marker_failed", error));
        return true;
      };

      const syncGovMapNeighborhoodBoundary = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!nativeMap || nativeMap.hidden || !window.govmap) {
          return false;
        }
        const boundary = payload?.govmap?.neighborhood_boundary;
        const objectId = boundary?.objectid || boundary?.source_object_id || boundary?.provenance_id || "address-neighborhood";
        const geometryName = `neighborhood-boundary-${govMapGeometryNamePart(objectId)}`;
        if (!govMapLayerToggleChecked(GOVMAP_SELECTED_NEIGHBORHOOD_LAYER)) {
          window.govmap.clearGeometriesByName?.([geometryName], GOVMAP_NATIVE_MAP_ID);
          return true;
        }
        const boundaryWkt = boundary?.display_wkt || boundary?.wkt;
        if (!boundaryWkt) {
          return false;
        }
        displayGovMapPolygonWkt({
          wkt: boundaryWkt,
          name: geometryName,
          tooltip: boundary?.label || "גבול שכונה",
          outlineColor: [30, 64, 175, 0.95],
          outlineWidth: 2,
          fillColor: [30, 64, 175, 0.04],
        }).catch((error) => console.warn("govmap_neighborhood_boundary_failed", error));
        return true;
      };

      const syncGovMapSelectedFocus = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const feature = payload?.govmap?.selected_focus_feature;
        const layer = String(feature?.govmap_layer || "").trim();
        const center = feature?.govmap_center || payload?.govmap?.center;
        if (!nativeMap || nativeMap.hidden || !window.govmap || !feature || !layer || !center) {
          return false;
        }
        const objectId = String(feature.source_object_id || feature.attributes?.objectid || "").trim();
        if (!govMapLayerToggleChecked(layer)) {
          govMapSelectedFocusRequestSeq += 1;
          window.govmap.clearSelection?.(GOVMAP_NATIVE_MAP_ID);
          return true;
        }
        if (!window.govmap.selectFeaturesOnMap || !Number.isFinite(Number(center.x)) || !Number.isFinite(Number(center.y))) {
          return false;
        }
        const requestSeq = ++govMapSelectedFocusRequestSeq;
        const pointWkt = `POINT(${Number(center.x)} ${Number(center.y)})`;
        const whereClause = objectId ? { [layer]: `(objectid = ${govMapWhereValue(objectId)})` } : undefined;
        waitForGovMapNativeCommand(window.govmap.selectFeaturesOnMap({
          continous: false,
          wkt: pointWkt,
          radius: 50,
          getWkt: false,
          filterLayer: false,
          layers: [layer],
          returnFields: { [layer]: ["objectid"] },
          selectOnMap: true,
          whereClause,
          isZoomToExtent: false,
        }, GOVMAP_NATIVE_MAP_ID)).catch((error) => {
          if (requestSeq !== govMapSelectedFocusRequestSeq) return;
          console.warn("govmap_selected_focus_failed", error);
        });
        return true;
      };

      const govMapFilterInputChecked = (type, value) => {
        const input = document.querySelector(`[data-map-${type}-toggle="${value}"]`);
        return !input || input.checked;
      };

      const applyGovMapOverlayFilters = () => {
        const overlay = document.getElementById("govmap-dashboard-overlay");
        if (!overlay) {
          return false;
        }
        for (const node of overlay.querySelectorAll("[data-map-layer], [data-map-object], [data-govmap-layer]")) {
          const layer = node.getAttribute("data-map-layer");
          const objectType = node.getAttribute("data-map-object");
          const govmapLayer = node.getAttribute("data-govmap-layer");
          const govmapLayerVisible = govMapLayerToggleChecked(govmapLayer);
          const visible = govmapLayerVisible && govMapFilterInputChecked("layer", layer) && govMapFilterInputChecked("object", objectType);
          node.style.display = visible ? "" : "none";
        }
        return true;
      };

      const updateMountedGovMapIframeUrl = (payload = window.__municipalDashboardGisMap) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const frame = nativeMap?.querySelector("iframe");
        if (!frame?.src || !payload?.govmap?.enabled) {
          return false;
        }
        const aliases = govMapCurrentAliases(payload);
        const url = new URL(frame.src);
        url.searchParams.set("lay", aliases.join(","));
        url.searchParams.set("laym", aliases.join(","));
        url.searchParams.set("z", String(payload.govmap.level || url.searchParams.get("z") || "7"));
      ...
      const govMapOfficialIframeUrl = (payload) => {
        if (!payload?.govmap?.iframe_url) return "";
        const aliases = govMapCurrentAliases(payload);
        const url = new URL(payload.govmap.iframe_url);
        url.searchParams.set("lay", aliases.join(","));
        url.searchParams.set("laym", aliases.join(","));
        url.searchParams.set("z", String(payload.govmap.level || url.searchParams.get("z") || "7"));
        return url.toString();
      ...
      const updateGovMapIframeUrl = (payload = window.__municipalDashboardGisMap) => {
        const officialFrame = document.getElementById("govmap-iframe");
        const officialUrl = govMapOfficialIframeUrl(payload);
        if (officialFrame && officialUrl && officialFrame.src !== officialUrl) {
          officialFrame.src = officialUrl;
        }
        if (syncGovMapNativeLayers(payload)) {
          applyGovMapOverlayFilters();
          return true;
        }
        return false;
      };

      window.__updateGovMapIframeUrl = updateGovMapIframeUrl;
      window.__applyGovMapOverlayFilters = applyGovMapOverlayFilters;

      const createNativeGovMap = async (payload) => {
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        if (!nativeMap || !payload?.govmap?.enabled) {
          return false;
        }
        const govmap = await ensureGovMapApi();
        const renderSeq = ++govMapNativeRenderSeq;
        nativeMap.hidden = false;
        nativeMap.replaceChildren();
        const center = payload.govmap.center || {};
        if (!Number.isFinite(Number(center.x)) || !Number.isFinite(Number(center.y))) {
          return false;
        }
        const iframeConfig = await loadGovMapIframeConfig();
        return new Promise((resolve) => {
          let settled = false;
          let mountedCheckTimer = null;
          const finish = (ready, error = null) => {
            if (settled || renderSeq !== govMapNativeRenderSeq) return;
            settled = true;
            if (mountedCheckTimer) window.clearInterval(mountedCheckTimer);
            if (error) console.warn("govmap_native_render_failed", error);
            if (ready) {
              govMapNativeCommandReady = true;
              syncGovMapNativeLayers(payload);
              window.setTimeout(() => syncGovMapNativeLayers(payload), 700);
              window.setTimeout(() => syncGovMapNativeLayers(payload), 1600);
              window.setTimeout(() => syncGovMapNativeLayers(payload), 4500);
              window.setTimeout(() => syncGovMapNativeLayers(payload), 9000);
            }
            resolve(Boolean(ready));
          };
          const settings = {
            token: iframeConfig?.token || payload.govmap.api_key,
            center: { x: Number(center.x), y: Number(center.y) },
            level: Number(payload.govmap.level || 7),
            background: Number(payload.govmap.background ?? 0),
            visibleLayers: govMapCurrentAliases(payload),
            layers: govMapCurrentAliases(payload),
            language: "he",
            ...
            onLoad: () => finish(true),
            onError: (event) => finish(false, event || new Error("govmap_native_on_error")),
          };
          try {
            Promise.resolve(govmap.createMap(GOVMAP_NATIVE_MAP_ID, settings)).then(() => finish(true)).catch((error) => finish(false, error));
          } catch (error) {
            finish(false, error);
          }
          const finishIfNativeMapMounted = () => {
            if (!nativeMap.hidden && nativeMap.childElementCount > 0) {
              const mountedFrame = nativeMap.querySelector("iframe");
              mountedFrame?.addEventListener("load", () => {
                govMapNativeCommandReady = true;
                syncGovMapNativeLayers(payload);
                window.setTimeout(() => syncGovMapNativeLayers(payload), 1200);
                window.setTimeout(() => syncGovMapNativeLayers(payload), 3500);
              }, { once: true });
              if (settled || renderSeq !== govMapNativeRenderSeq) return;
              settled = true;
              if (mountedCheckTimer) window.clearInterval(mountedCheckTimer);
          ...
          mountedCheckTimer = window.setInterval(finishIfNativeMapMounted, 500);
          window.setTimeout(() => finish(false, new Error("govmap_native_onload_timeout")), 30000);
        });
      };

      const govMapOverlayPosition = (payload, coordinates) => {
        const center = payload?.center;
        const radius = Number(payload?.govmap?.radius_m || payload?.query?.radius_m || 3000);
        if (!center || !Array.isArray(coordinates) || coordinates.length < 2) {
          return null;
        }
        const lat = Number(center.lat);
        const lon = Number(center.lon);
        const metersPerLon = Math.max(1, 111320 * Math.cos(lat * Math.PI / 180));
        const east = (Number(coordinates[0]) - lon) * metersPerLon;
        const north = (Number(coordinates[1]) - lat) * 110540;
        return {
          x: Math.max(8, Math.min(92, 50 + (east / radius) * 38)),
          y: Math.max(8, Math.min(92, 50 - (north / radius) * 38)),
        };
      };

      const govMapSelectedAreaRing = (payload) => {
        const geometry = payload?.parcel?.geometry || payload?.govmap?.selected_area?.geometry;
        if (geometry?.type === "Polygon") return geometry.coordinates?.[0] || [];
        if (geometry?.type === "MultiPolygon") return geometry.coordinates?.[0]?.[0] || [];
        return [];
      };

      const renderGovMapOverlay = (payload, options = {}) => {
        const overlay = document.getElementById("govmap-dashboard-overlay");
        if (!overlay) {
          return;
        }
        ...
        const polygonCoordinates = govMapSelectedAreaRing(payload);
        const points = polygonCoordinates.map((coordinate) => govMapOverlayPosition(payload, coordinate)).filter(Boolean);
        if (options.showSelectedArea !== false && points.length >= 3) {
          const polygon = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
          polygon.setAttribute("class", "govMapSelectedArea");
          polygon.setAttribute("data-map-layer", "parcel");
          polygon.setAttribute("data-map-object", "selected_area");
          polygon.setAttribute("data-govmap-layer", payload?.govmap?.selected_area?.layer || "neighborhoods_area");
          polygon.setAttribute("points", points.map((point) => `${point.x.toFixed(2)},${point.y.toFixed(2)}`).join(" "));
          svg.appendChild(polygon);
        }
        overlay.appendChild(svg);
        if (options.showSelectedArea !== false && points.length >= 3) {
          const labelPoint = points.reduce((acc, point) => ({ x: acc.x + point.x / points.length, y: acc.y + point.y / points.length }), { x: 0, y: 0 });
          const centerLabel = document.createElement("span");
          centerLabel.className = "govMapSelectedLabel govMapOverlayLabel";
          centerLabel.dataset.mapLayer = "parcel";
          centerLabel.dataset.mapObject = "selected_area";
          centerLabel.dataset.govmapLayer = payload?.govmap?.selected_area?.layer || "neighborhoods_area";
          centerLabel.textContent = payload?.parcel?.label || "אזור נבחר";
          centerLabel.style.left = `${labelPoint.x}%`;
          centerLabel.style.top = `${labelPoint.y}%`;
          overlay.appendChild(centerLabel);
        }
        if (options.showDistrictLabels !== false) {
          const districtLabels = [
            ["צפון העיר", 61, 7], ["מרכז העיר", 50, 34], ["מערב העיר", 20, 37], ["מזרח העיר", 82, 32], ["דרום העיר", 43, 88], ["חוף הים", 14, 14]
          ];
          for (const [label, x, y] of districtLabels) {
            const node = document.createElement("span");
            node.className = "govMapDistrictLabel govMapOverlayLabel";
            node.dataset.mapObject = "neighborhoods";
        ...
        const features = options.showMarkers === false ? [] : (Array.isArray(options.features) ? options.features : [
          ...(payload?.nearby_pois?.items || []),
          ...(payload?.layers?.environment?.items || []),
          ...(payload?.layers?.emergency?.items || []),
        ].slice(0, 28));
        for (const feature of features) {
          const position = govMapOverlayPosition(payload, feature.geometry?.coordinates);
          if (!position) continue;
          const marker = document.createElement("button");
          marker.type = "button";
          marker.className = "govMapOverlayMarker";
          marker.dataset.kind = feature.poi_category || "environment";
          marker.dataset.mapObject = mapObjectType(marker.dataset.kind);
          if (feature.govmap_layer) {
            marker.dataset.govmapLayer = feature.govmap_layer;
          }
          if (feature.id && feature.id === payload?.govmap?.selected_focus_feature?.id) {
            marker.classList.add("isSelectedFocus");
            marker.dataset.govmapFocus = "true";
          }
          marker.style.left = `${position.x}%`;
          marker.style.top = `${position.y}%`;
          marker.innerHTML = markerSvg(marker.dataset.kind);
          marker.title = `${featureTitle(feature)} · ${feature.govmap_layer || "GovMap"}`;
      ...
      const renderGovMapEmbed = (payload, options = {}) => {
        const mapNode = document.getElementById("real-gis-map");
        const mapFrame = document.querySelector(".mapFrame");
        const staticMap = document.getElementById("real-gis-static-map");
        const iframe = document.getElementById("govmap-iframe");
        if (!mapNode || !mapFrame || !iframe || !payload?.govmap?.iframe_url) {
          return false;
        }
        const nativeGovMapEnabled = options.nativeGovMapEnabled !== false;
        govMapNativeReady = false;
        govMapNativeCommandReady = false;
        window.__municipalDashboardGovMapNativeReady = false;
        payload.govmap.native_ready = false;
        if (staticMap) staticMap.hidden = true;
        mapNode.hidden = false;
        mapFrame.classList.add("hasRealGis", "govmapActive");
        mapFrame.classList.remove("maplibreActive");
        iframe.dataset.govmapZoom = String(payload.govmap?.level || 8);
        const officialUrl = govMapOfficialIframeUrl(payload);
        if (officialUrl && iframe.src !== officialUrl) {
          iframe.src = officialUrl;
        }
        iframe.hidden = false;
        iframe.onload = () => {
          if (!window.__municipalDashboardGovMapNativeReady) {
            setText("#map-renderer-status", "GovMap רשמי פעיל");
          }
        };
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const overlay = document.getElementById("govmap-dashboard-overlay");
        if (nativeMap) nativeMap.hidden = !nativeGovMapEnabled;
        if (overlay) {
          overlay.hidden = true;
          overlay.replaceChildren();
        }
        window.__municipalDashboardGisMap = payload;
        renderGovMapLayerControls(payload);
        updateGovMapFilterAvailability(true);
        const renderGovMapUnavailable = (error = null) => {
          if (error) console.warn("govmap_native_render_failed", error);
          govMapNativeReady = false;
          window.__municipalDashboardGovMapNativeReady = false;
          payload.govmap.native_ready = false;
          ...
          renderGovMapLayerControls(payload);
          setText("#map-renderer-status", "GovMap לא נטען");
          if (dashboardRoot) dashboardRoot.dataset.gisMapStatus = "govmap-native-unavailable";
        };
        if (nativeGovMapEnabled) {
          createNativeGovMap(payload).then((ready) => {
            if (ready) {
              govMapNativeReady = true;
              window.__municipalDashboardGovMapNativeReady = true;
              payload.govmap.native_ready = true;
              setText("#map-renderer-status", "GovMap רשמי פעיל");
              iframe.hidden = true;
              if (overlay) {
                overlay.hidden = true;
              ...
              syncGovMapNativeLayers(payload);
              window.setTimeout(() => syncGovMapNativeLayers(payload), 700);
              if (dashboardRoot) dashboardRoot.dataset.gisMapStatus = "govmap-native-rendered";
            } else {
              renderGovMapUnavailable(new Error("govmap_native_unavailable"));
            }
          }).catch((error) => {
            renderGovMapUnavailable(error);
          });
        } else {
          renderGovMapUnavailable(new Error("govmap_native_disabled"));
        }
        renderMapLayerCounts(payload);
        renderMapLayerSummary(payload);
        applyRealGisLegendLabels();
        setText("#map-renderer-status", nativeGovMapEnabled ? "GovMap רשמי פעיל" : "GovMap כבוי");
        setAttr(".mapPanel", "aria-label", "מפת GovMap לתושב");
        setText("#map-title", payload.title_he || "מפת GovMap לתושב");
        setText("#map-desc", mapExampleCopy(payload.selected_example));
        setText(".mapProvenanceBadgeTitle", "GovMap רשמי");
        setText(".mapProvenanceBadgeText", "המפה, הרקע והשכבות נטענים דרך GovMap הרשמי בלבד. אם GovMap לא נטען, לא מוצגת מפה אחרת.");
        setAttr(".mapProvenanceBadge", "data-spatial-representation", "govmap_embed");
        if (dashboardRoot) {
          dashboardRoot.dataset.realGisAvailable = "true";
          dashboardRoot.dataset.gisMapStatus = "govmap-rendered";
          dashboardRoot.dataset.mapExample = payload.selected_example || currentMapExample;
        }
        return true;
      };

      window.__zoomGovMapEmbed = (direction) => {
        const frame = document.querySelector(".mapFrame.govmapActive");
        if (!frame) {
          return false;
        }
        if (!document.getElementById(GOVMAP_NATIVE_MAP_ID)?.hidden && (window.govmap?.zoomIn || window.govmap?.zoomOut)) {
          if (Number(direction) > 0 && window.govmap.zoomIn) window.govmap.zoomIn(GOVMAP_NATIVE_MAP_ID);
          if (Number(direction) <= 0 && window.govmap.zoomOut) window.govmap.zoomOut(GOVMAP_NATIVE_MAP_ID);
          return true;
        }
        return false;
      };

      window.__recenterGovMapEmbed = () => {
        const frame = document.querySelector(".mapFrame.govmapActive");
        if (!frame) {
          return false;
        }
        const iframe = document.getElementById("govmap-iframe");
        if (iframe) iframe.dataset.govmapZoom = String(window.__municipalDashboardGisMap?.govmap?.level || 8);
        if (window.govmap?.zoomToXY && !document.getElementById(GOVMAP_NATIVE_MAP_ID)?.hidden) {
          const center = window.__municipalDashboardGisMap?.govmap?.center;
          if (center) {
            window.govmap.zoomToXY({ x: Number(center.x), y: Number(center.y), level: Number(window.__municipalDashboardGisMap?.govmap?.level || 7) }, GOVMAP_NATIVE_MAP_ID);
            return true;
          }
        }
        return false;
      ...
      const renderRealGisMap = (payload, options = {}) => {
        if (payload?.govmap?.iframe_url) {
          return renderGovMapEmbed(payload, options);
        }
        updateGovMapFilterAvailability(false);
      ...
      const loadDashboardGisMap = async (profile = "initial", options = {}) => {
        if (gisMapProfileRequest && profile === "initial") {
          return gisMapProfileRequest;
        }
        const run = (async () => {
        try {
          const mapProvider = options.provider || (currentMapExample === "tel_aviv_parcel" ? "local" : "govmap");
          const params = new URLSearchParams({
            example: currentMapExample,
            profile,
            provider: mapProvider,
          });
          const municipality = options.municipality || selectedMunicipalityForQuery();
          if (municipality && mapProvider === "govmap") {
            params.set("municipality", municipality);
          }
          if (options.address && mapProvider === "govmap") {
            params.set("address", options.address);
          }
          const focusLayer = options.focusLayer || (Array.isArray(options.govmapLayerAliases) ? options.govmapLayerAliases.find((alias) => String(alias || "").trim()) : null);
          if (focusLayer && mapProvider === "govmap") {
            params.set("focus_layer", String(focusLayer).trim());
          }
          const hasExplicitCenter = options.center_x !== null && options.center_x !== undefined && options.center_y !== null && options.center_y !== undefined && Number.isFinite(Number(options.center_x)) && Number.isFinite(Number(options.center_y));
          if (hasExplicitCenter && mapProvider === "govmap") {
            params.set("center_x", String(options.center_x));
            params.set("center_y", String(options.center_y));
          }
          if (options.timelineEventId && mapProvider === "govmap") {
            params.set("timeline_event_id", String(options.timelineEventId));
          }
          const response = await fetch(`${DASHBOARD_GIS_MAP_ENDPOINT}?${params.toString()}`, { headers: { Accept: "application/json" } });
          if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
          }
          const payload = await response.json();
          if (options.selectedGisStoryId && selectedGisStoryIdFromData() !== options.selectedGisStoryId) {
            return;
          }
          if (!options.selectedGisStoryId && selectedGisStoryIdFromData()) {
            return;
          }
          ...
          if (Array.isArray(options.govmapLayerAliases)) {
            payload.govmap = payload.govmap || {};
            payload.govmap.question_visible_layers = options.govmapLayerAliases;
          }
          if (Array.isArray(options.residentLayerKeys)) {
            payload.govmap = payload.govmap || {};
            payload.govmap.question_resident_layer_keys = options.residentLayerKeys;
          }
          if (payload.govmap) {
            const storyFiltersEnabled = options.storyFiltersEnabled === true && Boolean(selectedGisStoryIdFromData());
            payload.govmap.story_filters_enabled = storyFiltersEnabled;
            payload.govmap.selected_gis_story_id = storyFiltersEnabled ? selectedGisStoryIdFromData() : "";
            payload.govmap.default_visible_layers = [];
            if (!storyFiltersEnabled) {
              payload.govmap.question_visible_layers = [];
              payload.govmap.question_resident_layer_keys = [];
            }
          }
          const rendered = renderRealGisMap(payload, options);
          renderGovMapLayerControls(payload);
          if (options.gisSignature) {
            lastQuestionGisMapSignature = options.gisSignature;
            window.__municipalDashboardLastGisSignature = lastQuestionGisMapSignature;
          }
          if (dashboardRoot) {
      ...
      const stableGisArray = (value) => Array.from(new Set((Array.isArray(value) ? value : []).map((item) => String(item || "").trim()).filter(Boolean))).sort();

      const questionGisMapRequest = (data) => {
        const geo = data?.state?.intent_resolution?.geo || null;
        const focus = geo?.focus || null;
        const selectedStoryId = String(data?.state?.selected_gis_story_id || "").trim();
        if (!geo?.needs_gis || !focus) return null;
        const municipality = String(data?.main_civic_workspace?.map_context?.municipality_id || geo?.map_context?.municipality_id || selectedMunicipalityForQuery() || data?.state?.municipality_id || "").trim();
        const muniLabel = municipalitySearchLabel(municipality);
        const addressBase = String(focus.address_query || focus.place_query || "").trim();
        const address = addressBase && muniLabel && !addressBase.includes(muniLabel) ? `${addressBase} ${muniLabel}`.trim() : addressBase;
        const govmapLayerAliases = Array.isArray(geo.govmap_layer_aliases) ? geo.govmap_layer_aliases : [];
        const residentLayerKeys = Array.isArray(geo.resident_layer_keys) ? geo.resident_layer_keys : [];
        const centerX = Number(focus.center_x);
        const centerY = Number(focus.center_y);
        const hasExecutedCenter = Number.isFinite(centerX) && Number.isFinite(centerY);
        if (!govmapLayerAliases.length || (!address && !hasExecutedCenter)) return null;
        const selectedTimelineEventId = String(data?.state?.selected_timeline_event_id || "").trim();
        const gisSignature = JSON.stringify({
          address,
          municipality,
          selectedTimelineEventId,
          centerX: hasExecutedCenter ? centerX : null,
          centerY: hasExecutedCenter ? centerY : null,
          focusLayer: String(geo.focus_layer || "").trim(),
          govmapLayerAliases: stableGisArray(govmapLayerAliases),
          residentLayerKeys: stableGisArray(residentLayerKeys),
        });
        return { address, municipality, center_x: hasExecutedCenter ? centerX : null, center_y: hasExecutedCenter ? centerY : null, govmapLayerAliases, residentLayerKeys, focusLayer: geo.focus_layer, provider: "govmap", gisSignature, storyFiltersEnabled: Boolean(selectedStoryId), selectedGisStoryId: selectedStoryId, timelineEventId: selectedTimelineEventId, nativeGovMapEnabled: true };
      };

      const quickGovMapUrl = (request) => {
        if (!request || !Number.isFinite(Number(request.center_x)) || !Number.isFinite(Number(request.center_y))) return "";
        const aliases = stableGisArray(request.govmapLayerAliases || []);
        const url = new URL("https://www.govmap.gov.il/");
        url.searchParams.set("c", `${Number(request.center_x).toFixed(2)},${Number(request.center_y).toFixed(2)}`);
        url.searchParams.set("z", "10");
        url.searchParams.set("b", "0");
        url.searchParams.set("et", "1");
      ...
      const quickGovMapLayerGroups = (data, request) => {
        const aliases = stableGisArray(request?.govmapLayerAliases || []);
        const aliasSet = new Set(aliases);
        const storyLayers = Array.isArray(data?.main_civic_workspace?.map_context?.layers) ? data.main_civic_workspace.map_context.layers : [];
        const groups = [];
        const used = new Set();
        for (const storyLayer of storyLayers) {
          const activeAliases = (Array.isArray(storyLayer?.govmap_aliases) ? storyLayer.govmap_aliases : []).map((alias) => String(alias || "").trim()).filter((alias) => aliasSet.has(alias) && !used.has(alias));
          if (!activeAliases.length) continue;
          for (const alias of activeAliases) used.add(alias);
          groups.push({
            layer_key: storyLayer.layer_key || activeAliases.join("_"),
            display_name_he: storyLayer.display_name_he || storyLayer.layer_key || "GovMap",
            govmap_category_he: "GovMap",
            default_visible: true,
            layers: activeAliases.map((alias) => ({ alias, label_he: alias, kind: "map_layer", selectable: true, status: "primary" })),
          });
        }
        const remaining = aliases.filter((alias) => !used.has(alias));
        if (remaining.length) {
          groups.push({
            layer_key: "selected_story_govmap_layers",
            display_name_he: "שכבות הסיפור",
            govmap_category_he: "GovMap",
            default_visible: true,
            layers: remaining.map((alias) => ({ alias, label_he: alias, kind: "map_layer", selectable: true, status: "primary" })),
          });
        }
        return groups;
      };

      const quickGovMapPayload = (data, request) => {
        const url = quickGovMapUrl(request);
        const aliases = stableGisArray(request?.govmapLayerAliases || []);
        if (!url || !aliases.length || !request?.selectedGisStoryId) return null;
        return {
          status: "found",
          ...
          query: {
            ...
            center_x: Number(request.center_x),
            center_y: Number(request.center_y),
            timeline_event_id: request.timelineEventId,
          },
          govmap: {
            enabled: true,
            iframe_url: url,
            center: { x: Number(request.center_x), y: Number(request.center_y) },
            level: 10,
            visible_layers: aliases,
            question_visible_layers: aliases,
            question_resident_layer_keys: request.residentLayerKeys || [],
            resident_layer_groups: quickGovMapLayerGroups(data, request),
            default_visible_layers: [],
            story_filters_enabled: true,
            selected_gis_story_id: request.selectedGisStoryId,
            native_ready: false,
          },
          nearby_pois: { count: 0, displayed_count: 0, total_count: 0, items: [] },
          layers: {},
          coverage: {},
          legend: [],
        };
      ...
      const renderInstantStoryGovMap = (data, request) => {
        const payload = quickGovMapPayload(data, request);
        ...
        const nativeMap = document.getElementById(GOVMAP_NATIVE_MAP_ID);
        const overlay = document.getElementById("govmap-dashboard-overlay");
        if (!mapNode || !mapFrame || !iframe) return false;
        window.__municipalDashboardGisMap = payload;
        if (staticMap) staticMap.hidden = true;
        if (nativeMap) {
          nativeMap.hidden = true;
          nativeMap.replaceChildren();
        }
        if (overlay) {
          overlay.hidden = true;
        ...
        mapFrame.classList.add("hasRealGis", "govmapActive");
        mapFrame.classList.remove("maplibreActive");
        iframe.src = govMapOfficialIframeUrl(payload);
        iframe.hidden = false;
        renderGovMapLayerControls(payload);
        updateGovMapFilterAvailability(true);
        renderMapLayerCounts(payload);
        ...
        setText("#map-renderer-status", "GovMap רשמי פעיל");
        setAttr(".mapPanel", "aria-label", "מפת GovMap לתושב");
        setText("#map-title", payload.title_he);
        setText("#map-desc", mapExampleCopy(payload.selected_example));
        setText(".mapProvenanceBadgeTitle", "GovMap רשמי");
        setText(".mapProvenanceBadgeText", "המפה, הרקע והשכבות נטענים דרך GovMap הרשמי בלבד. אם GovMap לא נטען, לא מוצגת מפה אחרת.");
        setAttr(".mapProvenanceBadge", "data-spatial-representation", "govmap_embed");
        if (dashboardRoot) {
          dashboardRoot.dataset.realGisAvailable = "true";
          dashboardRoot.dataset.gisMapStatus = "govmap-official-iframe-rendered";
          dashboardRoot.dataset.gisMapProfile = "question";
          dashboardRoot.dataset.mapExample = payload.selected_example;
        }
        return true;
      };

      const loadQuestionGisMap = (data, request = questionGisMapRequest(data)) => {
        if (!request) return false;
        if (request.gisSignature && request.gisSignature === lastQuestionGisMapSignature && window.__municipalDashboardGisMap?.status === "found") {
          window.__municipalDashboardLastGisSkipped = true;
          syncGovMapNativeLayers(window.__municipalDashboardGisMap);
          return true;
        }
        window.__municipalDashboardLastGisSkipped = false;
        if (statusNode) {
          statusNode.textContent = "טוען מפת GovMap לפי מוקד השאלה.";
        }
        renderInstantStoryGovMap(data, request);
        loadDashboardGisMap("question", request);
        return true;
      };
      ...
      const renderDashboardFromEndpoint = (data) => {
        ...
        syncDetailDrawerToggle(drawerCopy.close_label || "סגירת תשובה");
        ...
        renderGisStoryReview(data.gis_story_review);
        ...
        const questionMapRequest = questionGisMapRequest(data);
        if (!questionMapRequest && window.__municipalDashboardGisMap?.status === "found" && !window.__municipalDashboardGisMapFromServer) {
          renderRealGisMap(window.__municipalDashboardGisMap);
        }
        if (questionMapRequest) {
          loadQuestionGisMap(data, questionMapRequest);
        } else if (!window.__municipalDashboardGisMapFromServer) {
          loadDashboardGisMap();
        }
        if (statusNode) {
      ...
      if (window.__municipalDashboardGisMapFromServer && window.__municipalDashboardGisMap?.status === "found") {
        const renderedServerMap = window.__municipalDashboardGisMap?.govmap?.iframe_url ? renderRealGisMap(window.__municipalDashboardGisMap) : false;
        if (!renderedServerMap) {
          renderMapLayerCounts(window.__municipalDashboardGisMap);
          renderMapLayerSummary(window.__municipalDashboardGisMap);
          renderGovMapLayerControls(window.__municipalDashboardGisMap);
          applyRealGisLegendLabels();
        }
        if (dashboardRoot) {
          dashboardRoot.dataset.realGisAvailable = "true";
          dashboardRoot.dataset.gisMapProfile = window.__municipalDashboardGisMap.query?.profile || "initial";
          dashboardRoot.dataset.gisMapStatus = renderedServerMap ? "server-govmap-rendered" : "server-svg-rendered";
        }
      }
      ...
      const resetDynamicMapToggleDefaults = () => {
        ...
        const govMapSelected = currentMapExample === "govmap_resident_point" || window.__municipalDashboardGisMap?.provider === "govmap";
        if (govMapSelected) {
          if (parcelLayerInput) parcelLayerInput.checked = false;
          if (parcelObjectInput) parcelObjectInput.checked = false;
          if (buildingInput) buildingInput.checked = false;
          if (waterInput) waterInput.checked = false;
          if (osmParksInput) osmParksInput.checked = false;
          window.__updateGovMapIframeUrl?.();
          return;
        }
        if (basemapInput) {
      ...
      const loadVisibleBuildings = async () => {
        ...
        try {
          const response = await fetch(appUrl(`/api/ui/rag-dashboard/gis-buildings?${params.toString()}`), { signal: visibleBuildingAbort.signal });
          if (!response.ok) {
      ...
      const loadVisibleParcels = async () => {
        ...
        try {
          const response = await fetch(appUrl(`/api/ui/rag-dashboard/gis-parcels?${params.toString()}`), { signal: visibleParcelAbort.signal });
          if (!response.ok) {
      ...
      const loadVisibleContextPois = async () => {
        ...
        try {
          const response = await fetch(appUrl(`/api/ui/rag-dashboard/gis-context-pois?${params.toString()}`), { signal: visibleContextPoiAbort.signal });
          if (!response.ok) {
      ...
      bindMapControlButton(mapViewZoomInButton, () => (realGisMap || document.querySelector(".mapFrame.govmapActive") ? zoomDashboardMap(1) : zoomViewport(0.72)));
      bindMapControlButton(mapViewZoomOutButton, () => (realGisMap || document.querySelector(".mapFrame.govmapActive") ? zoomDashboardMap(-1) : zoomViewport(1.38)));
      bindMapControlButton(mapViewResetButton, () => (realGisMap || document.querySelector(".mapFrame.govmapActive") ? recenterDashboardMap() : resetViewport()));
      ...
      mapFrame?.addEventListener("wheel", (event) => {
        event.preventDefault();
        event.stopPropagation();
        window.__municipalDashboardWheelZoomBlocked = true;
      }, { passive: false, capture: true });

      detailDrawerToggle?.addEventListener("click", () => {
        setDetailDrawerCollapsed(!detailDrawerCollapsed);
      });

      categorySection?.addEventListener("click", (event) => {
        const button = event.target.closest(".discoveryRow");
        if (button?.dataset.itemId && !button.disabled && !button.classList.contains("isDisabled")) {
      ...
      mapExampleSelect?.addEventListener("change", () => {
        currentMapExample = mapExampleSelect.value || "govmap_resident_point";
        loadDashboardGisMap();
      ...
      if (form) {
        form.addEventListener("submit", async (event) => {
          event.preventDefault();
          const question = String(questionInput && questionInput.value || "").trim() || "אילו החלטות עירייה התקבלו לאחרונה ליד דיזנגוף 99 תל אביב";
          if (questionInput) {
      ...
      if (popularButton && popularPopover) {
        ...
        popularPopover.addEventListener("click", (event) => {
          const choice = event.target.closest(".popularChoice");
          if (!choice) return;
  ...
  <script>
    (() => {
      ...
      const isGovMapActive = () => Boolean(realMap && !realMap.hidden && frame.classList.contains("govmapActive"));
      ...
      window.__applyGovMapOverlayScale = () => {
        ...
        const iconScale = Math.max(0.55, Math.min(1.75, 0.55 + (Number(window.__municipalMapIconScale || 0.05) * 2.0)));
        const labelScale = Math.max(0.7, Math.min(1.65, 0.62 + (Number(window.__municipalMapLabelScale || 0.08) * 1.15)));
        realMap.style.setProperty("--govmap-icon-scale", String(iconScale));
        realMap.style.setProperty("--govmap-label-scale", String(labelScale));
        return true;
      };
      ...
      const rerenderStaticMapWithCurrentFilters = () => {
        ...
        if (isGovMapActive()) {
          window.__updateGovMapIframeUrl?.();
          window.__applyGovMapOverlayFilters?.();
          updateToggleAllFiltersLabel();
          return;
        }
      ...
      const handleMapFilterChange = (input = null) => {
        syncParcelFilterInputs(input);
        rerenderStaticMapWithCurrentFilters();
        updateToggleAllFiltersLabel();
        return true;
      };
      window.__municipalDashboardMapFiltersChanged = handleMapFilterChange;
      for (const input of mapFilterInputs()) {
        input.addEventListener("change", (event) => {
          ...
          handleMapFilterChange(input);
        });
      }
      controls.popover?.addEventListener("change", (event) => {
        if (event.__municipalDashboardFilterHandled) return;
        event.__municipalDashboardFilterHandled = true;
        const input = event.target.closest?.("[data-map-layer-toggle], [data-map-object-toggle], [data-govmap-layer-toggle]");
      ...
      controls.iconSize?.addEventListener("input", (event) => {
        if (!isStaticMapActive() && !isGovMapActive()) {
          return;
        }
...
def _inject_initial_gis_map(html: str, payload: dict[str, Any]) -> str:
    if isinstance(payload.get("govmap"), dict) and payload["govmap"].get("iframe_url"):
        ...
        return html.replace("</head>", f"{payload_script}\n</head>", 1)
...
def _server_rendered_gis_svg(payload: dict[str, Any]) -> str:
    ...
    return f'''
            <svg id="real-gis-static-map" class="realGisStaticMap" viewBox="{focus_view_box}" data-world-view-box="{world_view_box}" data-focus-view-box="{focus_view_box}" data-geo-bounds="{geo_bounds}" role="img" aria-label="מפת GIS אמיתית ללא ספריית מפה חיצונית">
              ...
              {initial_layers}
              <path data-map-layer="parcel" data-map-object="selected_area" d="{selected_area_path}" fill="rgba(124,77,206,0.20)" stroke="rgba(255,255,255,0.96)" stroke-width="7" stroke-linejoin="round"><title>אזור נבחר - מעגל סביב הכתובת</title></path>
              <path data-map-layer="parcel" data-map-object="selected_area" d="{selected_area_path}" fill="rgba(124,77,206,0.22)" stroke="#7c4dce" stroke-width="2.6" stroke-linejoin="round"><title>אזור נבחר - מעגל סביב הכתובת</title></path>
