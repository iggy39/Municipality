...
from municipality.gis_importers import import_school_coordinates_csv  # noqa: E402
from municipality.gis_raw_storage import ImmutableRawStorage  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(description="Import Ministry of Education school coordinate CSV.")
    parser.add_argument("csv_path")
    parser.add_argument("--source-id", default="moe_school_coordinates")
    parser.add_argument("--raw-root", default="storage/raw/gis")
    args = parser.parse_args()
    engine = create_engine(os.environ["DATABASE_URL"], future=True)
    with Session(engine) as session, session.begin():
        summary = import_school_coordinates_csv(
            session,
